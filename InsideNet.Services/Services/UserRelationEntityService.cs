...
using InsideNet.Helpers.Helpers;
...
namespace InsideNet.Services.Services
{
    public class UserRelationsService : IUserRelationsService
    {
        ...
        public async Task<List<UserRelationDto>> GetAll(string userId)
        {

            var dtos = _mapper.Map<List<UserRelationDto>>(await _dbContext.UserRelationEntities
        ...
        public async Task<UserRelationDto> GetById(string currentUserId, string targetUserId)
        {
            return _mapper.Map<UserRelationDto>(await GetByCompositeIds(currentUserId, targetUserId)) ??
