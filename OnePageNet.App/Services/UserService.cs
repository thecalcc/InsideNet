...
public class UserService : IUserService
{
    ...
    public async Task AddAsync(UserDto entity)
    {
        ...
        await _dbContext.Users.AddAsync(_mapper.Map<ApplicationUser>(entity));
        await _dbContext.SaveChangesAsync();
    }

    public async Task<bool> AttachUser(UserDto entity)
    {
        var user = await _dbContext.Users.SingleOrDefaultAsync(x => x.Id == entity.Id);

        if (user?.Id != entity.Id || user == null) return false;
